...
function renderStories() {
    ...
    window.stories.forEach(story => {
        ...
        // Get story count for the user
        const storyCount = (window.storyDataMocks[story.id] || []).length;
        const maxRings = Math.min(storyCount, 3); // Limit to 3 rings for visual clarity
        const ringClasses = story.isYourStory ? 'your-story' : (story.hasNewStory ? 'has-story' : 'your-story');

        // Create rings based on story count
        let ringsHTML = '';
        for (let i = 0; i < maxRings; i++) {
            ringsHTML += `<div class="story-avatar-ring ${ringClasses}"></div>`;
        }
