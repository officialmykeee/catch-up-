...
// Function to render stories with dynamic ring generation
function renderStories() {
    ...
    window.stories.forEach(story => {
        ...
        // Generate dynamic ring gradient based on story count and viewed status
        let ringGradient = '';
        if (storyCount === 0) {
            // No stories - gray ring
            ringGradient = '#d1d5db';
        } else if (storyCount === 1) {
            // Single story - solid ring (purple if unviewed, gray if viewed)
            ringGradient = storyData[0].viewed ? '#d1d5db' : '#749cbf';
        } else if (storyCount > 1) {
            // Multiple stories - create segmented rings
            const segmentAngle = 360 / storyCount;
            const gapSize = 10; // degrees for gap between segments
            
            let gradientParts = [];
            for (let i = 0; i < storyCount; i++) {
                const startAngle = i * segmentAngle;
                const endAngle = startAngle + segmentAngle - gapSize;
                const gapEnd = startAngle + segmentAngle;
                
                // Use purple for unviewed, gray for viewed
                const segmentColor = storyData[i].viewed ? '#d1d5db' : '#749cbf';
                
                gradientParts.push(`${segmentColor} ${startAngle}deg ${endAngle}deg`);
                if (i < storyCount - 1) {
                    gradientParts.push(`transparent ${endAngle}deg ${gapEnd}deg`);
                }
            }
            
            ringGradient = `conic-gradient(from 0deg, ${gradientParts.join(', ')})`;
        }

        storyElement.innerHTML = `
            <div class="story-avatar-container">
                <div class="story-avatar-ring" style="background: ${ringGradient}" data-story-id="${story.id}">
...
// Export function to update story ring after viewing
window.updateStoryRing = function(storyId) {
    renderStories(); // Re-render all stories to update rings
};
