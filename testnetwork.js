...
function renderStories() {
    ...
    window.stories.forEach(story => {
        ...
        // Determine story count and ring class
        const storyCount = window.storyDataMocks[story.id]?.length || 0;
        let ringClass = story.isYourStory ? 'your-story' : 'no-story';
        if (!story.isYourStory && storyCount > 0) {
            if (storyCount === 1) {
                ringClass = 'has-story single-ring';
            } else if (storyCount === 2) {
                ringClass = 'has-story double-ring';
            } else if (storyCount >= 3) {
                ringClass = 'has-story triple-ring';
            }
        }
